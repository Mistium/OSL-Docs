...
Keys, expiry, payload lengths, session credentials, AWS path encoding, and HTTP
status are validated before signing or returning data. Requests reuse one
concurrency-safe HTTP client and its connection pool, plus shared transport-URL
and signed-header paths. `public_url` only affects `public()` output.
